...
Репозиторий поддерживает два отдельных сценария:

- `production-like Docker Compose` — базовый `docker-compose.yml` без bind mount и без `--reload`; этот сценарий сохраняет текущее поведение `docker compose up --build`.
- `local development` — dev-слой через `docker-compose.dev.yml`; инфраструктура остаётся в Docker, а API можно запускать либо в dev-контейнере с hot reload, либо локально из VS Code/терминала через `.env.local`.
...
`.env` нужен для Compose-сервисов и использует имена `postgres`, `redis`, `minio` внутри Docker network.
`.env.local` нужен только для локального запуска API на хосте и использует `127.0.0.1` для PostgreSQL, Redis и MinIO.
...
Это по-прежнему поднимает default HTTP stack: `postgres`, `redis`, `minio`, `minio-init`, `migrate`, `api` и `nginx`.
`worker` по-прежнему скрыт за профилем и не стартует по умолчанию.

Снаружи будет доступен Nginx на `http://localhost`.
MinIO S3 access по умолчанию привязан к loopback через `127.0.0.1:9000`.

В этом режиме rebuild нужен после изменений Python-кода, зависимостей, `Dockerfile.*` или системных пакетов, потому что исходники копируются внутрь image.
...
### Использовать Bearer token в Swagger

1. Откройте `/docs` и при необходимости переключите язык через кнопки `RU` / `EN`.
2. Нажмите `Authorize`.
...
### Проверить API локально через Bruno
